...
class ChatScreen extends StatefulWidget {
  ...
  const ChatScreen({super.key, required this.aiAssistantData});
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  void initState() {
    ...
    context.read<ChatProvider>().gptMessages = [];
  }
  ...
  void _sendMessage() async {
    ...
    context
        .read<ChatProvider>()
        .addMessages(text: _controller.text, sender: "user", isMe: true);
  ...
  Future<bool> backChecker() {
    if (context.read<ChatProvider>().isTyping) {
      ...
      if (context.read<ChatProvider>().messages.isNotEmpty) {
        context.read<ChatProvider>().insertDatabase(
          context: context,
